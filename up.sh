...
#
#  1. Create config:   cp .env.example .env
#                      nano .env
#  2. Start stack:     ./up.sh
...
if [ ! -f "$DIR/.env" ]; then
  ...
  echo "║  Edit it with admin info + API keys,     ║"
...
# Load config + secrets (single file now)
source "$DIR/.env"
...
[ -n "$OPENROUTER_API_KEY" ] && create_cred "OpenAI"     "openAIApi"     "{\"openAIApiKey\":\"$OPENROUTER_API_KEY\"}"
...
echo "║  Demo:  http://localhost:8080/home.html  ║"
