"""Zero-RAG baseline evaluation.

Identical metrics to ``eval_flowise.py`` (ROUGE, BLEU, BERTScore, LLM judge),
but the answers are generated by querying the Soofi model DIRECTLY — no
retrieval, no context, no RAG pipeline. This measures the model's parametric
knowledge alone and serves as a baseline to compare against the RAG system.
"""
...
# The Soofi endpoint uses a self-signed certificate.
...
# Direct model endpoint (no RAG). Same backend the Flowise pipeline uses.
MODEL_API_URL = "https://soofi-owu.l3s.de/api/chat/completions"
MODEL_API_KEY = "sk-ed443de23ce34d09b86ca3ef4df325f4"
MODEL_NAME = "Soofi-S-Instruct"

SYSTEM_PROMPT = (
    "You are a helpful assistant for ELSTER, the German electronic tax portal. "
...
def query(question: str) -> dict:
    """Query the model directly (no retrieval). Returns {"text": ...} or {"error": ...}."""
    headers = {
    ...
    for attempt in range(QUERY_MAX_ATTEMPTS):
        try:
            response = requests.post(
                MODEL_API_URL, headers=headers, json=payload, timeout=120, verify=False
            )
            data = response.json()
        except (requests.RequestException, ValueError) as e:
            last_error = {"error": f"request failed: {e}"}
        else:
            if response.status_code < 500 and "choices" in data:
...
def run_evaluation():
    ...
    print("Initializing zero-RAG baseline (direct model, no retrieval)...")
    ...
    for i, sample in enumerate(dataset):
        ...
        if failed:
            zero_judge = {
                ...
                "completeness": 0,
                "relevance": 0,
                "reasoning": "Model call failure — no prediction available",
            }
    ...
    summary = {
        "mode": "zero-RAG (direct model, no retrieval)",
        "model": MODEL_NAME,
    ...
    # Print summary
    print("\n" + "=" * 60)
    print("ZERO-RAG EVALUATION SUMMARY")
    print("=" * 60)
    print(f"Model:   {summary['model']} (no retrieval)")
